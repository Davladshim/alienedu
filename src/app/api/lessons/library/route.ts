...
// Библиотека готовых уроков — то, что другие репетиторы опубликовали
// (lessons.is_public = true), доступно для поиска и копирования себе.
// Собственные уроки в этой выдаче не показываем — добавлять то, что и так
// есть в своём списке, незачем
  ...
  try {
    ...
    const result = await query(
      `SELECT l.id, l.title, l.subject, l.grade, l.mode, l.created_at, u.full_name as author_name,
       ...
       WHERE l.is_public = true AND l.status = 'published' AND l.teacher_id != $1
