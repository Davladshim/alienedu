...
export async function POST(request: NextRequest) {
  try {
    ...
    const { student_id, date, time, duration_minutes, subject, notes } = await request.json()
    ...
    const result = await query(
      `INSERT INTO schedule_lessons (teacher_id, student_id, date, time, duration_minutes, subject, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id`,
      [decoded.id, student_id, date, time, duration_minutes || 60, subject || null, notes || null]
    )
